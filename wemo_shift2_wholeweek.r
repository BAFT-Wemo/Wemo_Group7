...
#CHECK ACCURACY ON TEST SET: RMSE 254.1252
...
# CHECK ACCURACY ON TEST SET. x is pred, y is actual. RMSE 241.1843
...
ar_forecast %>%
  ...
  geom_line(size=1)+
  labs(x='', title='ARIMA plot for [shift2] in [whole week]')
...
# CHECK ACCURACY ON TEST SET: RMSE 290.2837
...
#CHECK ACCURACY ON TEST SET. x is pred, y is actual. RMSE 497.2738
...
# CHECK ACCURACY ON TEST SET. x is pred, y is actual. RMSE 356.4401
...
full_df%>%
  ...
  labs(x='', title='Residuals for offline scooters in [shift2] on testing data in [whole week]')

...
#CHECK ACCURACY ON TEST SET: RMSE 432.2368
...
# CHECK ACCURACY ON TEST SET. x is pred, y is actual. RMSE 276.3122
...
ar_forecast %>%
  ...
  geom_line(size=1)+
  labs(x='', title='ARIMA plot for [shift3] in [whole week]')
...
# CHECK ACCURACY ON TEST SET: RMSE 339.6566
...
#CHECK ACCURACY ON TEST SET. x is pred, y is actual. RMSE 598.4562
...
# CHECK ACCURACY ON TEST SET. x is pred, y is actual. RMSE 441.5025
...
full_df%>%
  ...
  labs(x='', title='Residuals for offline scooters in [shift3] on testing data in [whole week]')

...
naive_df_s3 <- data.frame(forecast = rep(naive_pred_s3, 30),
                          admin_town_en = rep(naive_towns_s3, 30))
...
naive_forecast_date <- naive_s3 %>%
  inner_join(test_s3, by = c('service_hour_date', 'admin_town_en'))
...
#Train fitted
dist_train_naive <- data.frame(admin_town_en = train_s3$admin_town_en, 
                               sum_offline_scooter = train_s3$sum_offline_scooter, 
                               service_hour_date = train_s3$service_hour_date, 
                               shift = train_s3$shift, 
                               weekend_or_weekday = train_s3$weekend_or_weekday)

dist_train_naive$forecast[2:3458] <- train_s3$sum_offline_scooter[0:3457]
...
### Time Series Nest for [shift 3 + whole week] ###
...
ar_forecast %>%
  ...
  geom_line(size=1)+
  labs(x='', title='ARIMA plot for [shift3]')
...
#Valid  forecast
dist_valid_snaive <- data.frame(admin_town_en = test_s3$admin_town_en, 
                                sum_offline_scooter = test_s3$sum_offline_scooter, 
                                service_hour_date = test_s3$service_hour_date, 
                                shift = test_s3$shift, 
                                weekend_or_weekday = test_s3$weekend_or_weekday)

dist_valid_snaive$sum_offline_scooter[8:570] <- test_s3$sum_offline_scooter[0:563]
...
#Train fitted
dist_train_snaive <- data.frame(admin_town_en = train_s3$admin_town_en, 
                                sum_offline_scooter = train_s3$sum_offline_scooter, 
                                service_hour_date = train_s3$service_hour_date, 
                                shift = train_s3$shift, 
                                weekend_or_weekday = train_s3$weekend_or_weekday)

dist_train_snaive$forecast[8:3458] <- train_s3$sum_offline_scooter[0:3451]
...
full_df%>%
  ...
  labs(x='', title='Residuals for offline scooters in [shift3] on testing data')
...
plot.forecast <- function(full_data){
  plot <- full_data%>%
    ...
    facet_wrap(~admin_town_en, ncol =2, scale='free_y')+
    ylim(0, 2300)+
    labs(x='', title='Forecast / Actual for offline scooters in [shift3] on testing data')
  return(plot)
}
